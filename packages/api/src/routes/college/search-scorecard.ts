...
function transformCollegeData(college: CollegeData) {
  return {
    ...
    schoolCity: college["school.city"],
    schoolState: college["school.state"],
    ...
    satScoreAverage: college["latest.admissions.sat_scores.average.overall"],
    ...
    // Additional fields to match College type (set to null for API data)
    imageUrl: null,
    address: null,
    phone: null,
...
export const searchScorecardHandler = async (input: SearchScorecardInput) => {
  ...
  // Transform to frontend format and add rank based on position
  // Rank is only meaningful when not filtered by search (since we sort by SAT scores)
  const transformedColleges = limitedColleges.map((college, index) => ({
    ...transformCollegeData(college),
    rank: !input.search ? index + 1 : null,
  }));
